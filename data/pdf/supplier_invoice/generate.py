"""Generate supplier invoice PDFs from rows in DSQL.
    ...
    uv run python -m data.pdf.supplier_invoice.generate              # 50 PDFs to local_pdfs/
    uv run python -m data.pdf.supplier_invoice.generate --count 100  # more
    uv run python -m data.pdf.supplier_invoice.generate --s3         # upload to S3 instead
...
from data.pdf.supplier_invoice.layouts import Context, assign_layout, render
...
def main(count: int = 50, local_dir: str | None = _DEFAULT_LOCAL_DIR, to_s3: bool = False) -> dict:
    ...
    with Session(engine) as session:
        ...
        bucket = None
        if to_s3:
            bucket = os.environ["S3_INVOICES_BUCKET"]
            s3 = _s3_client()
        else:
