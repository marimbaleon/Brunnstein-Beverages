...
from data.documents.supplier_invoice._formatters import (
    format_date,
    format_eur,
    format_iban,
    format_qty,
...
def assign_layout(supplier: Supplier) -> int:
    ...
    Uses a simple character-sum hash — not cryptographic, just stable
    enough that each supplier always renders with the same template.
    """
...
# Shared structural constants. The visual styling (fonts, colors, borders)
# is intentionally NOT shared so each layout reads as a distinct template
# to the extraction model — but the column widths and recipient block are
# identical content, so they live here.

_ITEMS_COLUMN_WIDTHS = [1.2 * cm, 8.5 * cm, 2.3 * cm, 2.5 * cm, 2.5 * cm]
_TOTALS_COLUMN_WIDTHS = [5 * cm, 3.5 * cm]
...
def layout_classic(ctx: Context) -> bytes:
    ...
    items = Table(_line_rows(ctx.lines), colWidths=_ITEMS_COLUMN_WIDTHS)
    ...
    totals = Table(_totals_rows(ctx.invoice), colWidths=_TOTALS_COLUMN_WIDTHS, hAlign="RIGHT")
...
def layout_traditional(ctx: Context) -> bytes:
    ...
    banner = Table(
        [[
            ...
            Paragraph("Wirtschaftsbetrieb &middot; gegründet 1923", italic),
        ]],
        colWidths=[17 * cm],
    )
