...
Output lands in use_cases/automated_invoice_processing/test_invoices/<scenario>/ and is
committed so reviewers see the eval fixtures on GitHub.
"""
...
from data.documents.supplier_invoice.layouts import Context, render
...
_DEFAULT_OUT = "use_cases/automated_invoice_processing/test_invoices"
...
def _scenario_iban_fraud(po: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.flag_for_fraud_review,
        expected_signals=[Signal.iban_mismatch_full],
        context=ctx,
    )
...
def _scenario_iban_typo(po: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.flag_for_review,
        expected_signals=[Signal.iban_mismatch_typo],
        context=ctx,
    )
...
def _scenario_po_not_found(po: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.flag_for_review,
        expected_signals=[Signal.po_not_found],
        context=ctx,
    )
...
def _scenario_supplier_not_in_master(po: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.flag_for_review,
        expected_signals=[Signal.supplier_unknown, Signal.po_not_found],
        context=ctx,
    )
...
def _scenario_quantity_discrepancy(po: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.flag_for_review,
        expected_signals=[Signal.quantity_mismatch_vs_goods_receipt],
        context=ctx,
    )
...
def _scenario_price_drift(po: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.flag_for_review,
        expected_signals=[Signal.unit_price_drift],
        context=ctx,
    )
...
def _scenario_early_invoice(po_without_gr: PurchaseOrder) -> TestCase:
    ...
    return TestCase(
        ...
        expected_outcome=Decision.hold_for_goods_receipt,
        expected_signals=[Signal.no_matching_goods_receipt],
        context=ctx,
    )
...
def generate(out_dir: str = _DEFAULT_OUT) -> dict:
    ...
    for case in cases:
        ...
        meta = {
            ...
            "expected_signals": [str(s) for s in case.expected_signals],
